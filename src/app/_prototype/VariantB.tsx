...
// PROTOTYPE — Variant B: "Cartridge Shelf"
// Retro game-console vibe. Quiz packs are 3D-tilted cartridges on a shelf.
// Primary affordance is "load a cartridge" — quiz packs are objects, not list items.
...
function Cartridge({
  ...
  return (
    <div
      className="group relative w-[170px] shrink-0 cursor-pointer transition-transform duration-300 hover:-translate-y-3 hover:rotate-0"
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#1b1340] text-white">
      ...
      <header className="relative z-10 flex items-center justify-between px-8 py-6">
        <div className="flex items-center gap-2 font-black tracking-tight">
          ...
          <span className="text-lg">QUIZBOY</span>
          <span className="ml-2 rounded-md bg-white/10 px-2 py-0.5 font-mono text-[10px] tracking-widest text-white/70">
            v1.0
          </span>
        </div>
        <div className="flex items-center gap-2 text-sm">
          <button className="inline-flex items-center gap-1.5 rounded-lg border border-white/20 bg-white/5 px-3 py-1.5 text-white/80 hover:bg-white/10">
            <LogIn size={13} /> log in
          </button>
          <button className="inline-flex items-center gap-1.5 rounded-lg bg-yellow-300 px-3 py-1.5 font-bold text-black hover:bg-yellow-200">
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-8 pt-6 text-center">
        ...
        <p className="mx-auto mt-4 max-w-md text-sm text-white/60">
          Each pack is a self-contained quiz. Load one in, answer one question
          at a time, walk out smarter.
        </p>
      </section>
      ...
      <section className="relative z-10 mt-14 flex flex-col items-center pb-16 text-center">
        <button className="group inline-flex items-center gap-3 rounded-2xl bg-gradient-to-b from-yellow-300 to-amber-500 px-7 py-4 font-black tracking-wide text-black shadow-[inset_0_2px_0_rgba(255,255,255,.6),0_6px_0_rgba(0,0,0,.45)] transition active:translate-y-[3px] active:shadow-[inset_0_2px_0_rgba(255,255,255,.6),0_3px_0_rgba(0,0,0,.45)]">
